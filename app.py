...
def _sec_ua():
    """Real contact email for SEC's User-Agent (Streamlit secrets → env → fallback)."""
    try:
        if "SEC_EMAIL" in st.secrets:
            return str(st.secrets["SEC_EMAIL"])
    except Exception:
        pass
    return os.environ.get("SEC_EMAIL", "Indxx ETF Dashboard admin@indxx.com")
...
def load_news(feed_url=NEWS_FEED, limit=8):
    ...
    req = urllib.request.Request(feed_url, headers={"User-Agent": _sec_ua()})
